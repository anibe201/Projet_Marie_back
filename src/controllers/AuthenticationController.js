...
module.exports = {
    async register(req, res) {
        try {
            ...
            req.body.profileImage = "http://localhost:5000/public/user-image/default-man.png"
    ...
    async requestPasswordToken(req, res) {
        try {
            ...
            //     }
            // });
            res.send( 'You are receiving this because you (or someone else) have requested the reset of the password for your account.\n\n' +
                         'Please click on the following link, or paste this into your browser to complete the process:\n\n' +
                 'http://' + 'localhost:8080' + '/reset-password/' + token);
        } catch (err) {
            res.status(500).send({
